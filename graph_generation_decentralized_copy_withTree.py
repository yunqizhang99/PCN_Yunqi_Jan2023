...
testTree = randTreeList.buildTreeFromListAndGraph(node_list, G)
plt.clf()
posTree = nx.spring_layout(testTree)
nx.draw(testTree, posTree, with_labels = True)
plt.show(block=False)
plt.savefig("TestTreeForDecentralizedCopyWithTree.pdf", format="PDF")
...
while len(remain_demand_sum_list) < 10 or len(set(remain_demand_sum_list)) > 1:
    ...
    while len(nodes_i) > 0:
        ...
        while len(nodes_j) > 0:
            ...
            all_paths_i_j_sorted = []
            all_paths_i_j_sorted.append(list(nx.shortest_path(testTree, current_i, current_j)))
